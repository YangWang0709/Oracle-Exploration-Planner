...
The same photoreal clean image is also the preferred review background for automatic route candidates generated by `scripts/generate_oracle_routes.py`.
...
Automatic route generation overlays candidates on `photoreal_topdown_clean.png` and uses `photoreal_topdown_metadata.json` for world/pixel transforms. The review tool writes approved candidates to `approved_routes.jsonl`; only those approved routes should be converted to `approved_dense_trajectory.jsonl` and replayed with `route_source=auto_approved`.
